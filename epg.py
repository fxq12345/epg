...
def fetch(url,i):
    logging.info(f"【第{i}条】开始抓取: {url}")
    try:
        r = requests.get(url, headers=HEADERS, timeout=15)
        if r.status_code != 200:
            logging.error(f"【第{i}条】失败 状态码:{r.status_code}")
